...
async def new_game(ctx):
    ...
    if user_id in USER_CONFIGS:
        await ctx.author.send(
            "You currently have a game in progress. Do end_game! to finish your game."
        )
...
async def set_board_size(ctx, width: int, height: int):
    try:
        ...
        if int_w > 5 or int_w < 0:
            ...
            await ctx.send(
                f"You set the board size to be: {USER_CONFIGS[user_id].size[0]}x{USER_CONFIGS[user_id].size[1]}"
            )
...
async def set_challenges(ctx, *, data: str):
    try:
        ...
        if user_id not in USER_CONFIGS:
            await ctx.send(
                "You don't have an active game. Start one in a server with !new_game"
            )
            return

        if len(USER_CONFIGS[user_id].size) == 0:
            await ctx.send(
                "You need to have board size first before configuring the challenges. Set the size of the board with !board_size"
            )
            return
        ...
        if len(challenge_list) < board_size:
            await ctx.send(
                f"You challenge list has {len(challenge_list)} amount of challenges. It needs {board_size} challenges"
            )
            return
    ...
    try:
        ...
        for i, player_info in enumerate(players):
            if len(player_info) != 2:
                await ctx.send(
                    f"Player {i+1}'s info is wrong. Format: `player1_id player1_color, player2_id, player2_color`"
                )
                return
        ...
        await ctx.send(
            "Player IDs must be numbers. Format: `!players 123456789 red, 987654321 green`"
        )
    except Exception as e:
        print(f"An error occured: {e}")
...
async def load_config(ctx):
    try:
        ...
        if not ctx.message.attachments:
            await ctx.send(
                "Please attach a CSV file. Usage: `!load` with a .csv file attached"
            )
            return
        ...
        for line in lines:
            ...
            if keyword == "board":
                ...
                for i, entry in enumerate(player_entries):
                    ...
                    if len(player_info) != 2:
                        await ctx.send(
                            f"Player {i+1} format is wrong. Expected: `id color`"
                        )
                        return
                ...
                if len(USER_CONFIGS[user_id].size) == 0:
                    await ctx.send(
                        "Board size must be set before challenges. Make sure your CSV has a `board` line before `challenges`"
                    )
                    return

                challenge_list = parts[1:]
                board_size = (
                    USER_CONFIGS[user_id].size[0] * USER_CONFIGS[user_id].size[1]
                )

                if len(challenge_list) < board_size:
                    await ctx.send(
                        f"Not enough challenges. Need {board_size}, got {len(challenge_list)}"
                    )
                    return
                ...
                altered_challenge_list: list[str] = [
                    challenge.center(longest_challenge, "-")
                    for challenge in challenge_list
                ]
        ...
        await ctx.send(
            "Config loaded! Use `!all_info` to review, then `!start` when ready."
        )
...
async def start_game(ctx):
    ...
    try:
        if len(USER_CONFIGS[user_id].size) == 0:
            ...
            await ctx.send(
                "There are no challenges. Please set challenges with !challenges"
            )
...
async def set_board_size_error(ctx, error):
    if isinstance(error, commands.MissingRequiredArgument):
        await ctx.send(
            "Please provide both width and height! Usage: `!board_size <width> <height>`"
        )
